  ...
  const obras = (data ?? []).map(obra => {
    ...
    const total_venda = itens.reduce((soma, item) => {
      const calc = calcularItem({
        ...
        // fee por item é IGNORADO na listagem (hard-coded null) — o total da
        // lista sempre usa o fee padrão 1.02, exatamente como o cálculo antigo
        // (obras/page.tsx) e os KPIs do dashboard (lib/dashboard/metricas.ts).
        fee_mao_obra: null,
        fee_material: null,
